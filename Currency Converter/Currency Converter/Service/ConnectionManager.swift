...
//  Currency Converter
//
//  Created by Кирилл Клименков on 5/19/20.
...
class ConnectionManager: NSObject {
    ...
    static func stopNotifier() -> Void {
        do {
            ...
            try (ConnectionManager.shared.reachability).startNotifier()
        } catch {
    ...
    static func isReachable(completed: @escaping (ConnectionManager) -> Void) {
        if (ConnectionManager.shared.reachability).connection != .unavailable {
            completed(ConnectionManager.shared)
        }
    }
    ...
    static func isReachableViaWiFi(completed: @escaping (ConnectionManager) -> Void) {
        if (ConnectionManager.shared.reachability).connection == .wifi {
            completed(ConnectionManager.shared)
        }
    }
}
